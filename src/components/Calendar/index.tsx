...
export function Calendar({ selectedDate, onDateSelect }: CalendarProps) {
  ...
  const { data: blockedDates } = useQuery<BlockedDates>({
    queryKey: ['blocked-date', currentDate.year(), currentDate.month()],
    queryFn: async () => {
      const response = await api.get(`/users/${username}/blocked-dates`, {
        params: {
          year: dayjs(selectedDate).set('year', currentDate.year()),
          month: dayjs(selectedDate).set('month', currentDate.month()),
        },
      })
  ...
  const calendarWeeks = useMemo(() => {
    ...
    const previousMonthFillDay = Array.from({
      length: firstWeekDay,
    })
      .map((_, i) => {
    ...
    const nextMonthFillDay = Array.from({
